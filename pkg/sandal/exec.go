...
func Exec(c *config.Config, args []string, user, dir string,
	stdin io.Reader, stdout, stderr io.Writer) error {

	if c.VM != "" {
		return execViaMgmt(c.Name, args, user, dir, stdin, stdout)
	}

	return execNative(c, args, user, dir)
}

// execViaMgmt sends an exec request to the VM's embedded controller
// via the management socket and relays I/O.
func execViaMgmt(contName string, args []string, user, dir string,
