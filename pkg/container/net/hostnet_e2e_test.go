...
// End-to-end tests that exercise SplitHostNet + ResolveDynamic against a real
// netlink interface and walk the result through stringToAddrs + IPRequest.
//
...
// TestSplitHostNet_E2E_RealNetlink runs SplitHostNet end-to-end against a
// real netlink interface. No stub: this exercises upstreamIPv4()'s actual
// kernel lookup path.
func TestSplitHostNet_E2E_RealNetlink(t *testing.T) {
	setupDummyUpstream(t)

	in := "172.16.0.1/24,fd34:0135:0123:0:%uv4%::1/120,%uv6%:%uv4%::1/64"
	static, dynamic := SplitHostNet(in)

	wantStatic := []string{"172.16.0.1/24", "fd34:0135:0123:0:" + e2eIPv4Hextets + "::1/120"}
...
// for the STATIC half: template -> SplitHostNet -> stringToAddrs. Verifies
// the resulting bridge addresses are well-formed and that the IPv6 entry
// encodes the host IPv4 in its IID exactly where the design says it should.
func TestE2E_BridgeAddrFormation(t *testing.T) {
	setupDummyUpstream(t)

	static, _ := SplitHostNet("172.16.0.1/24,fd34:0135:0123:0:%uv4%::1/120")
	addrs, err := stringToAddrs(static[0] + "," + static[1])
	if err != nil {
...
// TestE2E_IPRequestWalksFromResolvedBridge confirms that the static-resolved
// bridge address feeds IPRequest correctly: first container allocates the
// next address (i.e. c0a8:632a::2) within the resolved /120.
...
// TestE2E_DynamicEntryResolution exercises the dynamic half: given a real
// upstream IPv4 (via the dummy) and an explicit upstream prefix, ResolveDynamic
// produces the public mirror address with both placeholders substituted.
func TestE2E_DynamicEntryResolution(t *testing.T) {
	setupDummyUpstream(t)

	_, prefix, _ := net.ParseCIDR("2a00:1d35:3b0a:4f00::/64")
	got := ResolveDynamic("%uv6%:%uv4%::1/64", prefix)
	want := "2a00:1d35:3b0a:4f00:" + e2eIPv4Hextets + "::1/64"
	if got != want {
		t.Fatalf("ResolveDynamic = %q, want %q", got, want)
	}
	wantIP := net.ParseIP("2a00:1d35:3b0a:4f00:" + e2eIPv4Hextets + "::1")
	gotIP, _, err := net.ParseCIDR(got)
	if err != nil {
		t.Fatalf("resolved value must parse: %v", err)
	}
	if !gotIP.Equal(wantIP) {
		t.Fatalf("resolved IP = %s, want %s", gotIP, wantIP)
	}
}

// TestE2E_PublicMirrorIIDPreservation simulates the renumber stamp for the
// FALLBACK path (no %uv6% in config): take the IID from the resolved ULA,
// combine with a sample upstream /64 prefix, and verify the resulting public
// address keeps the IPv4 hextets exactly where the design promises.
